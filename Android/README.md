...
- 通知渠道使用 `IMPORTANCE_DEFAULT`（**不是 LOW**）：LOW 会被系统归到「静默通知 / 更多通知」，
  不但被折叠，**MIUI/HyperOS 连状态栏小图标都不显示**；渠道本身不带声音与震动，所以不会打扰
- 若状态栏仍看不到小图标，检查小米的设置：`通知与控制中心 → 状态栏 → 有通知时显示图标`

### 关于「像网速那样在状态栏显示文字」

**做不到，这是 Android 的平台限制。** 状态栏由 SystemUI 自己绘制，第三方 App 没有任何 API
能在上面写文字，只能贡献一个单色小图标。系统设置里那条 `17.6 KB/s` 是 MIUI 自带的
「实时网速」，不是 App 实现的。

所以 Android 版提供了一个替代方案：

### 顶部悬浮价格条

- 用一个 `TYPE_APPLICATION_OVERLAY` 悬浮窗贴在**状态栏正下方**，常驻显示 `浙商 ↓ 928.47`，红涨绿跌
- 效果上等同于网速指示器；点击打开主界面，拖动可调整位置，位置会持久化
- 需要「显示在其他应用上层」权限（`SYSTEM_ALERT_WINDOW`），首次开启时 App 会引导授权
- 开关在「状态栏常驻 → 顶部悬浮价格条」，可随时关闭；旁边有「重置位置」
...
> ⚠️ **务必备份 `Android/keystore/goldpricebar.jks`。**
> 它被 `.gitignore` 忽略，所以 `git clean -fdx`、手动清理或换机器都会让它消失。
> 密钥一旦丢失，脚本会新建一把，而**新密钥签出的包无法覆盖安装旧版本**——
> 用户必须先卸载（会丢失已保存的成本价与提醒设置）。
> 每次构建都会打印 SHA-256 指纹，指纹变化就说明换了密钥。
> 正式发布建议把密钥放进上面的 CI Secrets，本地只做调试。
...
4. 想常驻看到价格文字，开启「顶部悬浮价格条」并授予「显示在其他应用上层」权限
