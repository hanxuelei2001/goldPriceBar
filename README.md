...
- 通知渠道用 `IMPORTANCE_DEFAULT` 而非 `LOW`：否则会被系统归到「静默通知 / 更多通知」，**MIUI/HyperOS 连状态栏小图标都不会显示**
- **顶部悬浮价格条**：Android 的状态栏由 SystemUI 绘制，第三方 App 无法在上面写文字（系统那条「实时网速」是 MIUI 自带功能），因此提供 `TYPE_APPLICATION_OVERLAY` 悬浮条贴在状态栏正下方常驻显示 `浙商 ↓ 928.47`，可拖动、可关闭
